...
def conv(image, kernel):
    ...
    for i in range(Hi):
        for j in range(Wi):
            out[i,j] = np.sum(padded[i:i+2*pad_width0+1, j:j+2*pad_width1+1]
                              * kernel)
    ### END YOUR CODE
...
#                 out[i, j] = 0
    ...
    for i in range(1, H-1):
        for j in range(1, W-1):
            opp = mat[theta[i,j]]
            tmp = [G[i+int(opp[0]), j+int(opp[1])], 
                   G[i+int(opp[2]), j+int(opp[3])]]
            if G[i, j] >= np.max(tmp):
                out[i, j] = G[i, j] 
    ### END YOUR CODE
